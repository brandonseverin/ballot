...
async function main() {
    ...
    const _newVoterAddress = parameters[1];
    if (!isAddress(_newVoterAddress)) throw new Error("New voter address not provided")
    const newVoterAddress = getAddress(_newVoterAddress)
